...
function TSM.GetItemValue(itemID, priceSource)
	...
	if priceSource == "Custom" then
		...
		if TSMAPI and TSMAPI.GetCustomPriceValue then
			return TSMAPI:GetCustomPriceValue(NM.db.profile.pricesource.customPriceSource, itemID)
		end

		-- TSM 4
        if TSM_API and TSM_API.GetCustomPriceValue then
			return TSM_API.GetCustomPriceValue(NM.db.profile.pricesource.customPriceSource, "i:" .. tostring(itemID))
        end
